...
`--mode inspect-mcp --timeout-ms 15000 --max-actions 24` and exactly this JSON stdin:
...
After settings dispatch is acknowledged, the script polls AT-SPI for a bounded
settings snapshot. It first checks for unique, visible, enabled `MCP` or
`MCP servers` navigation. Otherwise it visits the observed sidebar buttons
`Configuration`, `Plugins`, then `Connections`, at most once each and only when
uniquely present. Each page waits for MCP navigation or the exact `glean-eval`
label. Navigation can be activated only once, followed by a bounded wait for the
server row. Each wait uses at most 20 snapshots, 100 ms apart, within the overall
deadline. All action labels require whole-name matches after case and whitespace
normalization; page controls must have the observed `button` role.

The route uses at most five actions: open Settings, three page buttons, and MCP
navigation. It never fills a field, reads composer Text, clicks Send, selects a
surface, changes server settings, or uses coordinates. It never activates
Connect, Authenticate, Restart, Toggle, Add, Remove, or Enable. Missing, ambiguous,
disabled, or unrecognized navigation produces an inventory of the last page,
not an inferred status. Unknown metadata can reflect a page still loading.
...
- `visitedSettingsPages`: at most three fixed values from `Configuration`,
  `Plugins`, and `Connections`, in visit order. It contains no page content.
...
- `serverLabelMatchCount`, `serverRowObserved`, and `connectionStatus`.
  Status is `unknown` unless a fixed settings page or MCP navigation was activated
  and one exact server label has an explicit containing AT-SPI table row/list item with unambiguous fixed
