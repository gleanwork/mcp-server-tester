...
The command exits `0` only when every query succeeds. Use `queries` instead of `scenario` to run multiple queries. With `anthropic-computer-use`, MST submits all queries first, checks HITL prompts afterward, then collects native Claude telemetry for each query.
...
Each query result retains the native Claude session and trace metadata. The output file also contains a top-level `telemetry` array with response, tool calls, native usage/cost/timing, session correlation, artifacts, and evidence metadata. Computer Use action counts are diagnostics only and are not reported as Claude telemetry.
